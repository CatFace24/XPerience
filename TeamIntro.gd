...
#func _process(delta):
	#index = get_dialog_index() ##this is to get the dialog index from the parent(?) class
	
#func get_dialog_index():
	#return index

##function to set dialog index from parent(?) class
#func set_dialog_index(dialog_index):
	#index = dialog_index 
